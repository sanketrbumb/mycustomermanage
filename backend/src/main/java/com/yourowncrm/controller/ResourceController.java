...
@RestController @RequestMapping("/api/resources") public class ResourceController {
    ...
    public ResourceController(ResourceService service, JwtTokenProvider jwtProvider) {
    ...
    @GetMapping
    public List<Resource> getAll(@RequestHeader("Authorization") String t) {
        return service.getAll(tid(t));
    }
    @GetMapping("/{id}")
    public Resource getById(@RequestHeader("Authorization") String t, @PathVariable Long id) {
        return service.getById(tid(t), id);
    }
    @PostMapping
    public ResponseEntity<Resource> create(@RequestHeader("Authorization") String t,
                                       @RequestBody Map<String,Object> req) {
        return ResponseEntity.status(201).body(service.create(tid(t), req));
    }
    @PutMapping("/{id}")
    public Resource update(@RequestHeader("Authorization") String t,
